...
from outbound.speech.sarvam_tts import SarvamTTSManager
from outbound.speech.sarvam_stt import SarvamSTTManager
...
from outbound.call_store import (
    connect_db,
    get_db,
    count_users,
    create_user,
...
from core.storage import minio_enabled
...
# ---------------------------------------------------------------------------
# Batch-calling integration (see main.py / outbound/batching.py)
# ---------------------------------------------------------------------------
# Outbound calls placed by the batch engine (outbound/batching.py) flow through the SAME
...
def _ensure_batch_call_context(call_sid: str):
    ...
    if not call_sid or call_sid in active_calls:
        return
    try:
        from outbound.batching import service
...
async def recording_callback(request: Request):
    ...
    if recording_url and minio_enabled():
        try:
            import httpx
            from core.storage import upload_bytes, build_key
