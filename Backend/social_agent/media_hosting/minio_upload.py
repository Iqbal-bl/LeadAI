...
MINIO_PUBLIC_ENDPOINT must be an externally reachable HTTPS object endpoint.
...
def _client():
    ...
    verify_tls = os.getenv("MINIO_VERIFY_SSL", "true").lower() not in {"0", "false", "no"}
    return boto3.client(
        "s3",
...
def _public_url(bucket: str, key: str) -> str:
    ...
    base_url = _setting("MINIO_PUBLIC_ENDPOINT").rstrip("/")
