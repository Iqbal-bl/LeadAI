...
core.database = db_stub

# ---- stub `auth.get_current_user` -----------------------------------------
CURRENT_USER = {"email": "root@platform.io"}
auth_stub = types.ModuleType("core.auth")
auth_stub.get_current_user = lambda: CURRENT_USER["email"]
auth_stub.get_current_user_websocket = lambda ws: CURRENT_USER["email"]
sys.modules["core.auth"] = auth_stub
core.auth = auth_stub

# ---- stub `outbound.app` (twilio + SCRIPTS_DIR) ----------------------
mc_stub = types.ModuleType("outbound.app")
mc_stub.SCRIPTS_DIR = str(__import__("pathlib").Path(__file__).resolve().parent.parent / "scripts")
...
sys.modules["outbound.app"] = mc_stub
outbound.app = mc_stub

# ---- stub `globals`, `validate_number`, `db` (outbound) ------------------
g = types.ModuleType("outbound.call_state"); g.call_hangup_reasons = {}
sys.modules["outbound.call_state"] = g
outbound.call_state = g

vn = types.ModuleType("outbound.phone")
vn.validate_phone_number = lambda n: n if n.startswith("+") else "+91" + n
sys.modules["outbound.phone"] = vn
outbound.phone = vn

outbound_db = types.ModuleType("outbound.call_store")
...
sys.modules["outbound.call_store"] = outbound_db
outbound.call_store = outbound_db
...
import os as _os
script_files = [f for f in _os.listdir(mc_stub.SCRIPTS_DIR) if f.endswith(".xml")]
