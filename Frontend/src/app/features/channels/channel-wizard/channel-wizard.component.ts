...
export class ChannelWizardComponent implements OnInit, OnDestroy {
  ...
  private updatePlatformStates(): void {
    for (const platform of this.availablePlatforms) {
      if (platform.id === 'facebook' || platform.id === 'whatsapp') {
        ...
        platform.actionLabel = this._linkedinConnecting
          ? 'Waiting for Consent...'
          : 'Connect LinkedIn';
      }
    }
  ...
  getExternalIdPlaceholder(): string {
    switch (this.channelType) {
      case 'whatsapp':
        return 'e.g. 109876543210';
      case 'messenger':
        return 'e.g. 123456789012345';
      case 'instagram':
        return 'e.g. 17841401234567';
      case 'linkedin':
        return 'e.g. urn:li:person:CsjnS7Uz7f';
      default:
        return 'Enter ID';
    }
  }
    ...
    this.channelService
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          if (err.status === 409) {
            this.step1Error =
              'This external ID is already connected to a channel, possibly under a different company. Each number/page/account can only be connected once.';
          } else {
            this.step1Error =
              err.error?.detail ||
              err.message ||
              'Failed to create channel. Please check your inputs and try again.';
          }
        },
      });
  }
  ...
  submitTest(): void {
    ...
    this.channelService
      .testChannel(this.createdChannelId, {
        to: this.testTo.trim(),
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          this.testError =
            err.error?.detail ||
            err.message ||
            'Test message failed. Please verify your Meta credentials and 24h messaging session.';
        },
      });
  }
