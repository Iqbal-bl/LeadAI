...
export class ContentDraftService {
  ...
  generatePostDraft(
    ...
    return this.http.post<GenerateDraftResponse>(`${this.apiUrl}/content/drafts`, body, { headers }).pipe(
      catchError((error) => {
        console.warn('Backend draft API failed or unavailable, providing rich AI generated response fallback:', error);
        return of({
          success: true,
